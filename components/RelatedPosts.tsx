...
export function RelatedPosts({ posts }: RelatedPostsProps) {
  ...
  return (
    <section
      ...
      className="mt-16 pt-10 border-t border-[#16140F]/[0.08]"
    >
      <div className="text-[11px] tracking-[0.22em] uppercase text-[#9A4A26] font-semibold mb-7">
        You may also like
      </div>
          ...
          <li key={post.slug}>
            <Link href={`/blog/${post.slug}`} className="group block">
              {post.tags && post.tags.length > 0 ? (
                <div className="text-[10px] tracking-[0.18em] uppercase text-[#9A4A26]/80 font-semibold mb-2">
                  {post.tags[0]}
                </div>
              ) : (
                <div className="text-[10px] tracking-[0.18em] uppercase text-[#16140F]/40 font-semibold mb-2">
                  {formatDate(post.published_at)}
                </div>
              )}
              <h3
                className="text-[clamp(1.15rem,1.6vw,1.3rem)] leading-[1.25] tracking-[-0.01em] text-[#16140F] group-hover:text-[#9A4A26] transition-colors"
                style={{ fontFamily: "var(--font-playfair), serif" }}
              >
                {post.title}
              </h3>
              {post.summary && (
                <p className="text-[14px] text-[#16140F]/65 leading-[1.55] mt-2 line-clamp-3">
                  {post.summary}
                </p>
              )}
              <span className="inline-block mt-3 text-[11px] tracking-[0.16em] uppercase text-[#9A4A26] font-semibold">
                Read essay →
              </span>
            </Link>
          </li>
